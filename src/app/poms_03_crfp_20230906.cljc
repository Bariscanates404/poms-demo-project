...
#?(:cljs (defn project-table [data] [:> DataTable {:columns #js[#js {:name "project-id", :selector (fn [row] (.-id row)), :sortable true}
              ...
              (e/client
                (let [!state (atom {
                  ...
                  (dom/div (dom/props {:style {:position "relative" :top "50%" :left "0%" :overflow "auto" :height "500px" :width "750px" }})
                           ...
                           (with-reagent project-table (clj->js (e/server (project-data db))))
                           )
