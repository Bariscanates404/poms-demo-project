...
(defn supplier-information-data [db]
...
#?(:cljs (defn supplier-information-table [data] [:> DataTable {:columns #js[#js {:name "brand-name", :selector (fn [row] (.-brandname row)), :sortable true}
                  ...
                  (dom/form
                    ...
                    (dom/fieldset
                      ...
                      (dom/div (dom/props {:style {:position "relative" :top "50%" :left "5%"}})
                               (with-reagent supplier-information-table (clj->js (e/server (supplier-information-data db))))
                               )
